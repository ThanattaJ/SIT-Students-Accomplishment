...
<div>
    <template>
        <div v-for="(i,index) in chips" v-bind:key="index">
            <div v-if="active">
                <div :key="i" class="chips-wrap" :style="{top:`${- size -10 + sizeHalf}px`, left: `${sizeHalf}px`, height: `${size * 2 + 20}px`, transform: `rotate(${degOffset * i}deg)`}">
                    <icon-base class="chip iconchipEffect animated" :width="chipSize" :height="chipSize" :iconColor="randomColor()" :style="{left: `${- chipSize / 2}px`, width: `${chipSize}px`, height: `${chipSize}px`}">
                        <icon-chip />
                    </icon-base>
                </div>
            </div>
