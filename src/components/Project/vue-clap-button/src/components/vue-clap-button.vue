<template>
<div class="vclapbtn-bin-wrap" :style="{width : `${size}px`, height: `${size}px`}" @mouseenter="isAllHover = true" @mouseleave="isAllHover = false">
    <div class="counter bounceInUp animated" v-if="maxClick > 1" v-show="clickedTime > 0 && isHover" :style="{left: `${size / 2}px`, top: `-18px`, color: `${colorNormal}`}">
        +{{clickedTime}}
    </div>
    <icon-chips ref="chips" :size="size" />
    <div ref="circle" @click="click" @mouseenter="mouseenter" @mouseleave="mouseleave" :style="{
                width: `${size - 4}px`,
                height: `${size - 4}px`,
                margin: '2px',
                position: 'relative',
                ...
                lineHeight: `${size - 4}px`}" :class="{pulse: clicking}">
...
export default {
    ...
    methods: {
      ...mapActions([
        ...
        click() {
          // console.log('path',this.GET_PATHNAME)
          //   try {
          ...
          //       this.setClap(clap)
          //       console.log('can clap', this.getClap)
          //   } catch (err) {
